...
from data import FootballTable
...
from config import PROJECT_DIRECTIORY

class HistoricalSalaryTable(FootballTable):
    def __init__(self, refresh=True,  max_week=11):
        self.url_args = [(i +1, y) for y in ['2020', '2019', '2018'] for i in range(17)
                          if not (( i + 1 >max_week) and (y=='2020'))]
        super(HistoricalSalaryTable, self).__init__("historicalSalary", refresh)

    def build(self):
        out = []
        for wk, yr in tqdm(self.url_args):
            resp = requests.get("http://rotoguru1.com/cgi-bin/fyday.pl?week=%d&year=%s&game=dk&scsv=1" % (wk ,yr))
            ...
            sal = sal[sal.Week != ""]
            sal['name'] = sal.apply(lambda x: (x['Name'].split(',')[1] + x['Name'].split(',')[0]).upper()
...
class BacktestStandingsTable(FootballTable):
    ...
    def build(self):
        out = []
        for ix, link_row in tqdm(self.backtestTable.table.iterrows()):
            path = "%s/ref/Results/contest-standings-%s.csv" % (PROJECT_DIRECTIORY, link_row['gameid'])
            ...
            # Filter
            standings = pd.concat([standings[standings['Rank'] <= r].tail(1)
                                   for r in self.payoutTable.table[0].values])
            out.append(standings)
        self.table = pd.concat(out)


class DoubleupStandingsTable(FootballTable):
    def __init__(self, refresh=True):
        self.payoutTable = PayoutTable(refresh=True)
        self.backtestTable = BacktestLinksTable(refresh=True)
...
class BacktestPlayerPerformanceTable(FootballTable):
    def __init__(self, refresh=True):
        self.backtestTable = BacktestLinksTable(refresh=refresh)
        self.histSalaryTable = HistoricalSalaryTable(refresh=refresh)
    ...
    def build(self):
        ...
        for ix, link_row in tqdm(self.backtestTable.table.iterrows()):
            ...
            results['name'] = results.apply(lambda x: team_map_2[x['Player']] if x['Roster Position'] == "DST"
            else x['Player'], axis=1)
            ...
            results = results.join(self.histSalaryTable.table.set_index(['week', 'year', 'name']),
                                   on=['week', 'year', 'name'])
            out.append(results)
        self.table = pd.concat(out)


class BacktestPredictionsTable(FootballTable):
    def __init__(self, refresh=True):
        self.btPerf = BacktestPlayerPerformanceTable(refresh=refresh)
    ...
    def build(self, matchups=None, add_y=True):
        ...
        for nm, players in matchups.groupby(['year', 'week']):
            ...
            pp_matchups = players[(players['Roster Position'] != 'DST') & \
                                  (players['Roster Position'] != 'QB')][['name', 'date', 'opp']].copy()
            qb_fs = QuarterbackFeatureSpaceTable(refresh=False)
            pp_fs = PositionPlayerFeatureSpaceTable(refresh=False)
            df_fs = DefenseFeatureSpaceTable(refresh=False)
            ...
            # Build a feature space with matchups on the date (just select from table when equal date?)
            # We avoid this in case the last record in the feature space isnt the team they play against
            # I.E missing a week before
...
def run_doubleup_backtest(x, results, histStandings):
    num_teams_to_stack = x[0]
    num_players_in_stack = x[1]
    ...
    for team in stacked_teams:
        prior_lineups = []
        for i in range(int(20.0 / num_teams_to_stack)):
